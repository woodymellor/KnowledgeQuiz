...
if __name__ == '__main__':
    ...
    if not result.get('date'): raise SystemExit('ERROR: No internal sheet date found')
    if not result.get('validation',{}).get('ok'): raise SystemExit('ERROR: Parser validation failed')
    if not result.get('appearances'): raise SystemExit('ERROR: No appearances extracted')
    with open(args.output,'w',encoding='utf-8') as f:
        json.dump(result,f,indent=2,ensure_ascii=False)
    n=sum(len(a.get('questions',[])) for a in result['appearances'])
